...
export const interactiveMode = async () => {
  ...
  while (!done) {
    ...
    if (input === "1") {
      ...
      if (input === "1") {
        // Chorous One
        const networkMatch = await networkSanityCheck(chorus_one.ETH_MAINNET);
        if (!networkMatch) {
          console.error(
            `\n(Warning: Switch your orchestration network to Ethereum Hoodi (${chorus_one.ETH_MAINNET}) (.env) and reload the project to use Chrorus One staking)\n`
          );
          return;
        }
